...
openstack-keystone:                     active
    ...
    let(:fog_service) do
      Struct.new('FogService', :name).new('nova-compute')
    end

    describe "operations" do
      describe "#nova_compute_system_service" do
        let(:nova_compute_system_service) { FactoryGirl.create(:system_service, :name => 'openstack-nova-compute') }
        context "with nova compute system service present" do
          subject do
            host.system_services << nova_compute_system_service
            host.refresh_openstack_services(ssu)
            host.nova_compute_system_service
          end

          it { is_expected.to_not be_nil }
          it { is_expected.to be_eql nova_compute_system_service }
        end
      ...
      describe "#nova_compute_fog_service" do
        before(:each) do
          host.refresh_openstack_services(ssu)
        end

        subject do
          host.nova_compute_fog_service
        end

        context "without ems_cluster" do
          it { is_expected.to be_nil }
        end

        context "with ems_cluster" do
          before(:each) do
            host.ems_cluster = FactoryGirl.create(:ems_cluster)
          end

          context "without cloud" do
            it { is_expected.to be_nil }
          end

          context "with cloud" do
            before(:each) do
              infra_cluster = FactoryGirl.create(:ems_cluster_openstack)
              host.ems_cluster = infra_cluster

              provider = FactoryGirl.create(:provider_openstack, :name => "undercloud")
              @ems_infra = FactoryGirl.create(:ems_openstack_infra_with_authentication, :name => "undercloud", :provider => provider)
              @cloud = FactoryGirl.create(:ems_openstack_with_authentication, :name => "overcloud").tap do |fc_cloud|
                allow(fc_cloud).to receive(:openstack_handle).and_return('foo')
                #allow(fc_cloud).to receive(:save).and_return(true)
              end
              binding.pry
              # run the next line in the pry session and test will pass
              # just exit pry session or comment out binding.pry and test will fail
              # bundle exec rspec spec/models/manageiq/providers/openstack/infra_manager/host_spec.rb:127
              @ems_infra.provider.cloud_ems << @cloud
              host.ems_cluster.ext_management_system = @ems_infra
            end

            #it { is_expected.to_not be_nil }
            it { expect(@ems_infra.provider.cloud_ems.first).to equal @cloud }
            it { expect(@ems_infra.provider.cloud_ems.first.openstack_handle).to eq 'foo' }
          end
        end
      end

      describe "#nova_compute_enable_scheduling" do
      end
    end
